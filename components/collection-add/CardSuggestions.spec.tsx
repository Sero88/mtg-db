...
const suggestedCardNames = ["Test Suggestion 1", "Test Suggestion 2", "Test Suggestion 3"];
...
describe("CardSuggestions component", () => {
	it("should display suggested cards", () => {
		render(
			<CardSuggestions
				cards={suggestedCardNames}
				limit={suggestedCardNames.length}
				selectionHandler={selectionHandler}
			/>
		);
		expect(screen.queryAllByText(/Test Suggestion [0-9]/).length).toEqual(
			suggestedCardNames.length
		);
	});
